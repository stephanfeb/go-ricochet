...
| 3 | Operator mailbox observability | **Mostly done** (B2). `Storage.ServerStats` is the first cross-owner aggregate on the interface: mailbox and message totals, payload bytes, database size, mailbox-depth distribution, and how many mailboxes sit at their own cap. Published as `ricochet_mailboxes`, `ricochet_mailbox_depth{bucket}`, `ricochet_mailboxes_near_capacity` and friends, and it also fixed `queryCapacity`, which reported storage as 100% free regardless of what was on disk. What remains is the per-mailbox operator *listing* — "show me the top 20 by depth" — which is B3. |
