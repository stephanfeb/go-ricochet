...
| 3 | Operator mailbox observability | **Done** (B2 + B3). `Storage.ServerStats` is the first cross-owner aggregate on the interface: mailbox and message totals, payload bytes, database size, mailbox-depth distribution, and how many mailboxes sit at their own cap. Published as `ricochet_mailboxes`, `ricochet_mailbox_depth{bucket}`, `ricochet_mailboxes_near_capacity` and friends, and it also fixed `queryCapacity`, which reported storage as 100% free regardless of what was on disk. **Now done** (B3). `/ops/mailboxes/top?n=20` lists the fullest mailboxes across every owner, `/ops/mailboxes?owner=<peerID>` scopes it to one peer without being that peer, and `/ops/storage` totals per owner. Each row carries `messageCount`, `maxMessages`, `fillRatio` and a `full` verdict, so "is this mailbox full?" is one `curl` and no division. |
